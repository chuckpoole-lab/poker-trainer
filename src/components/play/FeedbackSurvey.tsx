    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--on-surface, #1a1a1a)', marginBottom: 8 }}>Thank you!</div>
          ...
          <button onClick={onClose} style={{
            ...
            background: '#4a7c59', color: '#fff', border: 'none', borderRadius: 12, cursor: 'pointer',
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--on-surface, #1a1a1a)' }}>Quick feedback</div>
          <div style={{ fontSize: 13, color: '#64748b', marginTop: 4 }}>Takes less than 30 seconds</div>
        </div>

        {/* Q1: First time? */}
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--on-surface, #1a1a1a)', marginBottom: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {[{ val: 1, label: 'Yes' }, { val: 2, label: 'No' }].map(opt => (
              <button key={opt.val} onClick={() => setFirstTime(opt.val)} style={{
                ...
                fontFamily: 'var(--font-body, inherit)',
                background: firstTime === opt.val ? '#4a7c59' : 'var(--surface-container, #f1f5f9)',
                color: firstTime === opt.val ? '#fff' : '#64748b',
                border: firstTime === opt.val ? '2px solid #4a7c59' : '2px solid var(--outline-variant, #e2e8f0)',
              }}>{opt.label}</button>
            ))}
          </div>
        </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {[{ val: 1, label: 'Meh', emoji: '😐' }, { val: 2, label: 'Yeah!', emoji: '😊' }, { val: 3, label: 'Loved it', emoji: '🤩' }].map(opt => (
              <button key={opt.val} onClick={() => setFun(opt.val)} style={{
                ...
                borderRadius: 12, cursor: 'pointer', transition: 'all 0.15s',
                fontFamily: 'var(--font-body, inherit)',
                background: fun === opt.val ? '#4a7c59' : 'var(--surface-container, #f1f5f9)',
                color: fun === opt.val ? '#fff' : '#64748b',
                border: fun === opt.val ? '2px solid #4a7c59' : '2px solid var(--outline-variant, #e2e8f0)',
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {[{ val: 1, label: 'Probably not', emoji: '👋' }, { val: 2, label: 'Maybe', emoji: '🤔' }, { val: 3, label: 'Definitely', emoji: '🔥' }].map(opt => (
              <button key={opt.val} onClick={() => setReturning(opt.val)} style={{
                ...
                borderRadius: 12, cursor: 'pointer', transition: 'all 0.15s',
                fontFamily: 'var(--font-body, inherit)',
                background: returning === opt.val ? '#4a7c59' : 'var(--surface-container, #f1f5f9)',
                color: returning === opt.val ? '#fff' : '#64748b',
                border: returning === opt.val ? '2px solid #4a7c59' : '2px solid var(--outline-variant, #e2e8f0)',
              }}>{opt.emoji} {opt.label}</button>
            ))}
          </div>
        </div>
        ...
        <button onClick={handleSubmit} disabled={!anyContent} style={{
          width: '100%', padding: 14, fontSize: 15, fontWeight: 700,
          background: anyContent ? '#4a7c59' : '#94a3b8', color: '#fff',
...
export function WelcomeToast({ onDismiss }: { onDismiss: () => void }) {
  return (
    <div style={{
      ...
      padding: '12px 16px', background: 'linear-gradient(135deg, #4a7c59 0%, #059669 100%)',
